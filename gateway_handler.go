...
func newGatewayHandler(c GatewayConfig, nd *Node) *gatewayHandler {
...
func (i *gatewayHandler) getOrHeadHandler(w http.ResponseWriter, r *http.Request) {
	...
	dr, err := i.api.Get(r.Context(), resolvedPath)
	if err != nil {
	...
	*/
	...
	idx, err := i.api.Get(r.Context(), ipath.Join(resolvedPath, "index.html"))
	switch err.(type) {
	case nil:
...
func (i *gatewayHandler) servePretty404IfPresent(w http.ResponseWriter, r *http.Request, parsedPath ipath.Path) bool {
	...
	dr, err := i.api.Get(r.Context(), resolved404Path)
	if err != nil {
		return false
	}
	defer dr.Close()
