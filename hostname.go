...
func hostnameHandling(api *Api, mux *http.ServeMux, knownGatewaysMap map[string]*config.GatewaySpec, dnslink bool) (*http.ServeMux, error) {
	childMux := http.NewServeMux()

	knownGateways := prepareKnownGateways(knownGatewaysMap)
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if gw, ok := isKnownHostname(host, knownGateways); ok {
			...
			if hasPrefix(r.URL.Path, gw.Paths...) {
				...
				if gw.UseSubdomains {
					...
					newURL, err := toSubdomainURL(host, r.URL.Path, r, api)
					if err != nil {
		...
		if gw, gwHostname, ns, rootID, ok := knownSubdomainDetails(host, knownGateways); ok {
			...
			if rootCID, err := cid.Decode(rootID); err == nil {
				...
				if !strings.HasPrefix(r.Host, dnsCID) {
					dnsPrefix := "/" + ns + "/" + dnsCID
					newURL, err := toSubdomainURL(gwHostname, dnsPrefix+r.URL.Path, r, api)
					if err != nil {
						http.Error(w, err.Error(), http.StatusBadRequest)
						return
					}
				...
				if isPeerIDNamespace(ns) {
					if rootCID.Type() != cid.Libp2pKey {
						newURL, err := toSubdomainURL(gwHostname, pathPrefix+r.URL.Path, r, api)
						if err != nil {
							http.Error(w, err.Error(), http.StatusBadRequest)
							return
						}
		...
		if dnslink && isDNSLinkName(r.Context(), api, host) {
			// rewrite path and handle as DNSLink
			r.URL.Path = "/ipns/" + stripPort(host) + r.URL.Path
			childMux.ServeHTTP(w, withHostnameContext(r, host))
			return
...
func isDNSLinkName(ctx context.Context, api *Api, host string) bool {
	...
	_, err := api.NameResolve(ctx, name, depth)
...
func toSubdomainURL(hostname, path string, r *http.Request, api *Api) (redirURL string, err error) {
